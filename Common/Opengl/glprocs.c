...
static const void WINAPI InitGetExtensionsStringARB (HDC hdc)
{
	void *extproc;
	...
	if (extproc == NULL) 
	{
		assert(0 && "wglGetExtensionsStringARB entry point not found!!");
		_ASSERT(0);
		return;
	}
...
static const void WINAPI InitGetExtensionsStringEXT (void)
{
	void *extproc;
	...
	if (extproc == NULL) 
	{
		assert(0 && "wglGetExtensionsStringEXT entry point not found!!");
		_ASSERT(0);		
		return;
	}
