...
doRedraw(void)
{
    ...
    if (renderToDIB) {
	glFinish();
	BitBlt(hDCWindow, 0, 0, winWidth, winHeight, hDC, 0, 0, SRCCOPY);
	GdiFlush();
    } else {
	SwapBuffers(hDC);
    }
...
setupDIB(HDC hDC)
{
    ...
    bitsPerPixel = colorIndexMode ? 8 : GetDeviceCaps(hDC, BITSPIXEL);
	...
	break;
    case 16:
	bmHeader->biCompression = BI_BITFIELDS;
	bmHeader->biSizeImage = 0;
...
resizeDIB(HDC hDC)
{
    ...
    setupDIB(hDC);
    // This is necessary so the context will notice that the DIB has changed
...
setupPalette(HDC hDC)
{
    ...
    */
	...
	rmask = ((1<<pfd.cRedBits)-1)   << pfd.cRedShift;
	gmask = ((1<<pfd.cGreenBits)-1) << pfd.cGreenShift;
	bmask = ((1<<pfd.cBlueBits)-1)  << pfd.cBlueShift;
	return;
    }
...
setupPixelFormat(HDC hDC)
{
    ...
    pfd.cColorBits = colorIndexMode ? 8 : GetDeviceCaps(hDC, BITSPIXEL);
    ...
    int nCmdShow)
{
    ...
    /* Set up for OpenGL rendering.
     * The operations which follow must be performed in a specific order,
     * or palette remapping may not occur properly.
     */

    hDCWindow = GetDC(hWnd);

    /* hDC refers to the device context into which we will render */
    hDC = renderToDIB ? CreateCompatibleDC(hDCWindow) : hDCWindow;

    /* Set up the pixel format and palette, but do not select the palette. */
    setupPixelFormat(hDC);
    setupPalette(hDC);

    /* Select the palette into the window first, to ensure that palette
     * remapping will be performed correctly when we blit.
     */
    SelectPalette(hDCWindow, hPalette, FALSE);
    RealizePalette(hDCWindow);

    /* If rendering to a DIB, select the palette first, then the bitmap. */
    if (renderToDIB) {
	SelectPalette(hDC, hPalette, FALSE);
	RealizePalette(hDC);

	/* Create the DIB and select into the device context */
	setupDIB(hDC);
    }

    /* Create the rendering context, bind it, and away we go! */
