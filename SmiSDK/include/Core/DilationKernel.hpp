...
/*
		// Example
		PGCore::DilationKernel<char, T> dilationKernel(1);
		{			
 ...
 ***/	
	...
	{	
		...
		{
			memcpy(m_buffer, iKernelObject.GetBuffer(), iDimension*iDimension*sizeof(T));					
		}
 ...
 ***/
	...
	template <class T, class U>
	bool DilationKernel<T, U>::drawCircle(int iXc, int iYc, int iRadius, char * ioBuf)
	{
		if (!ioBuf) return false;

		memset(ioBuf, 0, m_dimension*m_dimension*sizeof(char));
		if (m_dimension==3)
		{
						  ioBuf[1] = 1;
	...
	template <class T, class U>
	bool DilationKernel<T, U>::convolve(const Image<U>& iImage, Image<U>& oImage) const
	{	
	...
	template <class T, class U>
	bool DilationKernel<T, U>::convolve(const BitImage<U>& iImage, BitImage<U>& oImage) const
	{	
		//LOG1("Begin Convolve %d.", &iImage);
		int r, c, cc, rr, center;            
			...
			for(c=0;c<cols;c++)
			{
				...
				for(rr=(-center);rr<=center;rr++)
				{
					for(cc=(-center);cc<=center;cc++)
					{
						...
						if(((c+cc) >= 0) && ((c+cc) < cols) && ((r+rr) >= 0) && ((r+rr) < rows))
						{
							U iVal = iImage.GetValue(r+rr, c+cc);
							if (
