...
#ifndef _PG_VOLUME_HPP_
#define _PG_VOLUME_HPP_
//////////////////////////////////////////////////////////////////////////
...
{
	PGCore::BitVolume<T>& maskVol = GetBitVolume(iImageIndex);
...
bool Volume<T>::DilateBitVolume(const int iImageIndex/*=0*/, const int iVoxels/*=1*/)
{
	if (iVoxels)
	{
		...
		PGCore::DilationKernel<char, T> dilationKernel(iVoxels);
		PGCore::BitVolume<T>& maskVol = GetBitVolume(iImageIndex);
...
bool Volume<T>::ErodeBitVolume(const int iImageIndex/*=0*/, const int iVoxels/*=1*/)
{
	if (iVoxels)
	{
		...
		PGCore::ErosionKernel<char, T> erosionKernel(iVoxels);
		PGCore::BitVolume<T>& maskVol = GetBitVolume(iImageIndex);
		PGMath::Point3D<long> mDims;
		maskVol.GetDimensions(mDims);
...
template <class T>
BitVolume<T>& Volume<T>::GetBitVolume(const int iImageIndex/*=0*/)
{
...
template <class T>
bool Volume<T>::GetMoments(double* oMean, double* oVariance/*=0*/) const
{
		...
		for (int k=0; k< zSize; k++)
		{
			...
			for (int j=0; j<ySize; j++)
			{
				for (int i=0; i<xSize; i++)
				{
					...
					variance += ((double)((nextVal - *oMean)*(nextVal - *oMean))
						* pixelDenom);
...
{
	double iMean=-1.0f, iVariance=0.0f;
