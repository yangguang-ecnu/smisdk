...
#if !defined(AFX_IODicom3D_H__C4EA1B91_0881_44C7_927E_3CD2769137C9HPP__INCLUDED_)
#define AFX_IODicom3D_H__C4EA1B91_0881_44C7_927E_3CD2769137C9HPP__INCLUDED_
		...
		PGCore::Volume<T> *oDataObject)
{
	...
	oMetaData->SetPatientName(firstMetaData.GetPatientName());
	...
	PGMath::Vector3D<float> firstOrVecX = nextOrVecX[0], firstOrVecY = nextOrVecY[0];
	...
	//PGCore::DilationKernel<char, T> dilationKernel(1); bool dilatedOK=false;
	...
	PGCore::BitImage<T> bImage(iRows, iColumns);
	PGCore::BitImage<T> sbImage(iRows, iColumns);
	...
	for (int i=0; i<iSlices; i+=m_skipFactorZ)
	{
		...
		int zLim = 1;
#if (_SMOOTH_MT_==1)
		if (GetSmoothFlag()) zLim = m_skipFactorZ;
#endif
		for (int j=0; j<zLim; j++)
		{
			...
			const std::vector<PGMath::Vector3D<float> >& nextOrVecsX = nextMetaDataObject.GetImageOrientationsPatientX();
			double dProdX = firstOrVecX*nextOrVecsX[0];
			if (abs(dProdX-1.0f) > 0.01f)
			{
				LOG0("IODicom3D::Read: WARNING: Image orientation X mismatch!");						
				continue;
			}

			const std::vector<PGMath::Vector3D<float> >& nextOrVecsY = nextMetaDataObject.GetImageOrientationsPatientY();
			double dProdY = firstOrVecY*nextOrVecsY[0];
			if (abs(dProdY-1.0f) > 0.01f)
			{
				LOG0("IODicom3D::Read: WARNING: Image orientation Y mismatch!");						
				continue;
			}
...
#ifdef _DEBUG
		char fileName[256] = {0};
		_snprintf(fileName, 255, "C:\\Tmp\\Dump\\OriginalImage_%04d.pgm", i); fileName[255] = '\0';
		...
		{			
			...
			binarizer.GetOutput(static_cast<PGCore::BitImage<T> *>(&bImage));				
...
{ 
	...
	//: PGIO::kPgIOSourceTypeFolder;		
	{
		...
		// set smoothing flag
		pDICOMImgIO3D.SetSmoothFlag(iParams->smoothWhenSkipping);
...
#endif // !defined(AFX_IODicom3D_H__C4EA1B91_0881_44C7_927E_3CD2769137C9HPP__INCLUDED_)
