...
namespace PGAlgs
{
	...
	long GLRendererBase<T, U>::GetMaximum3DTextureMemory()
	{
		...
		if(isIt)
		{
			glGetIntegerv(GL_MAX_3D_TEXTURE_SIZE, &(txSize));
			cout<<"Max 3D texture size: "<<txSize<<endl;
			LOG1("Max 3D texture size: %d", txSize);	
		}
		else 
		{
	...
	void GLRendererBase<T, U>::MouseClickCB(int button, int state, int x, int y )
	{
		...
		if (PgStaticRenderer)  
		{
			(static_cast<GLRendererBase<T, U> * >(PgStaticRenderer))->SetSkipFactor(gSparseMode); 			

			if (state == GLUT_UP)
			{
				if ((static_cast<GLRendererBase<T, U> * >(PgStaticRenderer))->GetRendererType() ==kPgRendererTypeMPR)
				{
					(static_cast<GLRendererBase<T, U> * >(PgStaticRenderer))->UpdateRender(-1, 1);
				}
			}
		}
